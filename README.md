...
| Follow-up com IA | A cada passo da régua, a IA (grátis: `GROQ_API_KEY`, `OPENROUTER_API_KEY` ou `GEMINI_API_KEY`; paga: `ANTHROPIC_API_KEY`) escreve a mensagem daquela paciente no campo "Follow-up · mensagem", com trava de segurança; o robô da etapa envia. Sem chave ou se a IA errar, vai a mensagem aprovada. |
